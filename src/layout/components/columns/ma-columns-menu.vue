<template>
  <div
    class="sider customer-scrollbar flex flex-col items-center bg-gray-800 dark:border-blackgray-5"
  >
    ...
    <ul class="mt-1 parent-menu-container">
      <template v-for="(bigMenu, index) in userStore.routers" :key="index">
        <liuserStore :class="`${classStyle}`" @click="loadMenu(bigMenu, index)">
          <component
            v-if="bigMenu.meta.icon"
            :is="bigMenu.meta.icon"
            class="text-xl mt-1"
          />
          <span
            class="mt-0.5"
  ...
  <div class="layout-menu shadow flex flex-col" v-show="showMenu">
    <div class="menu-title flex items-center" v-show="!appStore.menuCollapse">
      {{ title }}
    </div>
    <a-layout-sider
      :style="`width: ${
        appStore.menuCollapse ? '50px' : appStore.menuWidth + 'px'
      };
