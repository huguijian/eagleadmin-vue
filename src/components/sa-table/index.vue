      ...
      <div ref="crudHeaderRef">
        <template v-if="showSearch">
          <a-row>
            ...
            />
            <a-col
              :flex="singleLine ? '185px' : '80px'"
              :style="{ textAlign: 'right' }"
            >
              <a-space
                :direction="!singleLine ? 'vertical' : 'horizontal'"
                :size="singleLine ? 10 : 20"
              >
          ...
          <a-col :span="16">
            <a-space>
              ...
              <a-popconfirm
                ...
                v-if="
                  !isRecovery && options.delete.show && options.rowSelection
                "
              >
                <a-button
                  type="primary"
              ...
              <a-popconfirm
                ...
                v-if="
                  options.recovery.show && isRecovery && options.rowSelection
                "
              >
                <a-button
                  type="primary"
          ...
          >
            <a-space v-if="options.showTools">
              ...
              <a-tooltip content="显隐搜索">
                <a-button shape="circle" @click="searchChange"
                  ><icon-search />
                </a-button>
              </a-tooltip>
                    ...
                    <a-table-column :title="row.title">
                      <template v-for="(rowChild, indexChild) in row.children">
                        <a-table-column
                          ...
                          :tooltip="
                            rowChild.dataIndex === '__operation'
                              ? false
                              : rowChild.tooltip ?? true
                          "
                        ...
                        >
                          <template #cell="{ record, column, rowIndex }">
                            ...
                            <template
                              v-else-if="rowChild.dataIndex === '__operation'"
                            >
                            ...
                            >
                              <a-tag
                                :color="
                                  tool.getColor(
                                    record[rowChild.dataIndex],
                                    dictList[rowChild.dict] ?? [],
                                    rowChild.colors || []
                                  )
                                "
                                >{{
                            ...
                            <template v-else-if="rowChild.type === 'image'">
                              <a-avatar
                                @click="
                                  imageSee(rowChild, record, rowChild.dataIndex)
                                "
                                :size="row.size || 64"
                                shape="square"
                              >
                  ...
                  >
                    <template #cell="{ record, column, rowIndex }">
                      ...
                      <template v-else-if="row.type === 'image'">
                        ...
                        >
                          <img
                            :src="imageView(record[row.dataIndex])"
                            style="object-fit: contain; cursor: pointer"
                          />
                        </a-avatar>
                      </template>
...
const getIndex = (rowIndex) => {
  ...
  if (requestParams.value["page"] === 1) {
    ...
    return (
      (requestParams.value["page"] - 1) * requestParams.value["limit"] + index
    );
  }
};
...
const init = async () => {
  ...
  if (options.value.singleLine) {
    ...
    singleLine.value =
      Object.getOwnPropertyNames(props.searchForm).length > 3 ? false : true;
  }
  currentApi.value =
    isRecovery.value &&
...
const requestData = async () => {
  ...
  if (isFunction(currentApi.value)) {
    ...
    if (response.data && response.data.items) {
      ...
      tableData.data = response.data.items;
    } else {
      tableData.total = 0;
...
const deleteAction = async (record) => {
  const params = { id: [record[options.value.pk]] };
  if (isRecovery.value) {
    // 销毁
    if (
      options.value.delete.realFunc &&
      isFunction(options.value.delete.realFunc)
    ) {
...
const deletesMultipleAction = async () => {
  const params = { id: selecteds.value };
...
const recoverysMultipleAction = async () => {
  ...
  if (selecteds.value && selecteds.value.length > 0) {
    if (
      options.value.recovery.func &&
      isFunction(options.value.recovery.func)
    ) {
      options.value.recovery.func(params);
    } else {
      console.error(`sa-table error：crud.recovery.func is not Function.`);
...
const settingFixedPage = () => {
  const workAreaHeight = options.value.height
    ? options.value.height
    : document.querySelector(".work-area").offsetHeight;
  let tableHeight =
    workAreaHeight - headerHeight.value - 160 + (!showSearch.value ? 15 : 0);
