    ...
    <div class="lg:w-10/12 w-full">
      ...
      >
        ...
        <template #tableSearch>
          <a-col :span="7">
            <a-form-item field="file_name" label="原文件名">
              <a-input
        ...
        <template #tools>
          <a-tooltip
            :content="mode === 'list' ? '切换橱窗模式' : '切换列表模式'"
          >
                ...
                <template v-for="record in tableData.data" :key="record.id">
                  <div class="mb-2 image-content">
                    ...
                    <a-image
                      ...
                      :description="`大小：${record.size}`"
                      :src="
                        /image/g.test(record.path)
                          ? tool.attachUrl(record.url)
                          : $url + 'not-image.png'
                      "
                    >
                      <template #extra>
                        <div class="actions">
                          <a-tooltip content="下载此文件">
                            <span class="action" @click="download(record)"
                              ><icon-download
                            /></span>
                          </a-tooltip>
                          <a-tooltip>
                            ...
                            <template #content>
                              <div>
                                存储名称：{{ record.md5_file }}.{{ record.ext }}
                              </div>
                              <div>存储目录：{{ record.path }}</div>
                              <div>上传时间：{{ record.create_time }}</div>
                              <div>
                                存储模式：{{
                                  tool.getLabel(
                                    record.storage_mode,
                                    dictList["attachment_type"]
                                  )
                                }}
                              </div>
                            </template>
        ...
        <template #url="{ record }">
          <a-image
            class="list-image"
            v-if="/image/g.test(record.path)"
            width="40px"
            height="40px"
            :src="tool.attachUrl(record.url)"
          />
          <a-avatar v-else shape="square" style="top: 0px">{{
            record.suffix
          }}</a-avatar>
        </template>
        <template #md5_file="{ record }">
          <div>{{ record.md5_file }}.{{ record.ext }}</div>
        </template>
        <!-- 操作列前置扩展 -->
...
const download = async (record) => {
  const response = await commonApi.downloadById({ id: record.id });
  if (response) {
    tool.download(response, record.file_name);
