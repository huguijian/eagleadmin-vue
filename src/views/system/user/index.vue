    ...
    <div class="lg:w-10/12 w-full">
      ...
      >
        ...
        <template #tableSearch>
          <a-col :span="8">
            <a-form-item field="user_name" label="账号名称">
              <a-input
        ...
        <template #avatar="{ record }">
          <a-avatar>
            <img
              :src="
                record.avatar
                  ? $tool.showFile(imageHost + record.avatar)
                  : $url + 'avatar.jpg'
              "
        ...
        <template #operationAfterExtend="{ record }">
          ...
          >
            ...
            <template #content>
              <!-- <a-doption value="updateCache" v-auth="['/admin/user/cache']"
                >更新缓存</a-doption
              >
              <a-doption
                value="setHomePage"
                v-auth="['/admin/user/setHomePage']"
                >设置首页</a-doption
              > -->

              <a-doption
                value="resetPassword"
                v-auth="['/admin/user/init-password']"
                >重置密码</a-doption
              >
            </template>
          </a-dropdown>
...
const updateCache = (id) => {
  api
    .clearCache({ id })
    .then((res) => res.code === 0 && Message.success(res.msg));
};
...
const saveHomePage = async (done) => {
  const resp = await api.setHomePage({
    id: userid.value,
...
const options = reactive({
  ...
  delete: {
    ...
    realAuth: ["/admin/user/realDestroy"],
...
const columns = reactive([
  ...
  { title: "账户", dataIndex: "user_name", width: 130 },
