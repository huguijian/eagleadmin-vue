  ...
  <div class="ma-content-block lg:flex justify-between">
    ...
    >
      ...
      <template #operationBeforeExtend="{ record }">
        <a-space
          size="mini"
          v-if="record.id > 1 && !record.disabled && !isRecovery"
        >
          <a-link
            v-auth="['/admin/role/update-menu-permission']"
            @click="openMenuList(record)"
            ><icon-menu /> 菜单权限</a-link
          >
          <a-link
            v-auth="['/admin/role/update-data-permission']"
            @click="openDataScopeList(record)"
            ><icon-layers /> 数据权限</a-link
          >
        </a-space>
      </template>
    </sa-table>
...
const options = reactive({
  ...
  delete: {
    ...
    realAuth: ["/admin/role/realDestroy"],
