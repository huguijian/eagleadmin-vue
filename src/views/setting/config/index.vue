<template>
  <div class="ma-content-block lg:flex justify-between">
      ...
      <a-list :max-height="650" :scrollbar="true" size="small">
        ...
        <a-list-item v-for="(item, index) in configGroupData">
          <div class="flex justify-between items-center">
            <a-button
              :type="title == item.name ? 'outline' : ''"
              @click="getConfigData(item.id, item.name, item.code)"
            >
              {{ item.name }}({{ item.code }})
            </a-button>
            <div class="flex">
              <a-link
                v-auth="['/core/config/update']"
                @click="editGroupModal(item)"
              >
                <template #icon>
                  <icon-edit />
                </template>
        ...
        <div class="pl-4 pr-4">
          <a-form :model="form" auto-label-width>
            ...
            <a-form-item v-if="formArray.length > 0">
              <a-button type="primary" @click="submit(formArray)"
                >保存修改</a-button
              >
            </a-form-item>
            <a-form-item
              label="测试邮件"
              v-if="currentNode.code === 'email_config'"
            >
              <a-input v-model="email" placeholder="请输入正确的邮箱接收地址" />
              <a-button type="primary" class="ml-2" @click="sendMail()"
                >发送</a-button
              >
            </a-form-item>
          </a-form>
    ...
    <a-modal
      ...
      type="warning"
      :on-before-ok="deleteConfigGroup"
    >
      <template #title>危险操作</template>
      <div class="mb-2">
        确定要删除
        <span class="text-red-500 underline font-black">{{
          deleteGroupData.name
        }}</span>
        配置组吗？
      </div>
      <div>
        此操作会删除组和所属组配置项，如果执行请在下面输入框输入：<span
          class="text-red-500"
          >{{ deleteGroupData.name }}</span
        >
      </div>
      <a-input
        :placeholder="`请输入 ${deleteGroupData.name}`"
        class="mt-2"
        v-model="name"
      />
    </a-modal>
...
const deleteConfigGroup = async (done) => {
  ...
  const response = await config.deleteConfigGroup({
    id: deleteGroupData.value.id,
  });
  if (response.code === 0) {
...
const getConfigGroupList = async () => {
  ...
  configGroupData.value = response.data.items;
...
const getConfigData = async (id, name, code) => {
  ...
  formArray.value = response.data.items;
  if (id === 2) {
